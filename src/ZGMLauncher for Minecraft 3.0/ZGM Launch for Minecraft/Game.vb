...
Module Game
    Dim library_list_forge_180() As String = {
            ...
            "libraries\net\minecraft\launchwrapper\1.11\launchwrapper-1.11.jar",
            "libraries\net\minecraftforge\forge\1.8-11.14.3.1450\forge-1.8-11.14.3.1450.jar",
    ...
    Dim library_list_forge_1710() As String = {
        "libraries\net\minecraftforge\forge\1.7.10\forge.jar",
        "libraries\net\minecraft\launchwrapper\1.11\launchwrapper-1.11.jar",
        "libraries\org\ow2\asm\asm-all\5.0.3\asm-all-5.0.3.jar",
        "libraries\com\typesafe\akka\akka-actor_2.11\2.3.3\akka-actor_2.11-2.3.3.jar",
        "libraries\com\typesafe\config\1.2.1\config-1.2.1.jar",
        ...
        "libraries\org\lwjgl\lwjgl\lwjgl\2.9.1\lwjgl-2.9.1.jar",
        "libraries\org\lwjgl\lwjgl\lwjgl_util\2.9.1\lwjgl_util-2.9.1.jar",
        "libraries\tv\twitch\twitch\5.16\twitch-5.16.jar",
        "versions\1.7.10\1.7.10.jar"
        }

    Function getCommand(gamePath As String, gamePath_ As String, name As String, version As String)

        If version = "180forge" Then
            Return command180forge(gamePath, gamePath_, name)
        ElseIf version = "180" Then
            Return command180(gamePath, gamePath_, name)
    ...
    Function command180(gamePath As String, gamePath_ As String, name As String)
        ...
        Dim c As String = "-XX:HeapDumpPath=MojangTricksIntelDriversForPerformance_javaw.exe_minecraft.exe.heapdump -Xmx1538M " &
                          "-XX:+UseConcMarkSweepGC -XX:+CMSIncrementalMode -XX:-UseAdaptiveSizePolicy -Xmn128M -Djava.library.path=" &
                          """" & gamePath & "Djava\lite" & """ -cp "

        For Each library In library_list_forge_180
            c &= """" & gamePath & library & """;"
        Next

        c &= " " & " net.minecraft.launchwrapper.Launch --username " & name & _
                 " --version 1.8 --gameDir """ & gamePath_ & """ --assetsDir """ & gamePath & "assets"" --assetIndex 1.8 --accessToken myaccesstoken --userProperties {} "
        Return c
    End Function
    Function command180forge(gamePath As String, gamePath_ As String, name As String)
        Dim library As String
        ...
        c &= " " & " net.minecraft.launchwrapper.Launch --tweakClass com.mumfrey.liteloader.launch.LiteLoaderTweaker --username " & name & _
    ...
    Function command1710forge(gamePath As String, gamePath_ As String, name As String)
        Dim library As String
        ...
        For Each library In library_list_forge_1710
            c &= """" & gamePath & library & """;"
        Next

        c &= " " & " net.minecraft.launchwrapper.Launch --username " & name & _
            " --version 1.7.10 --gameDir """ & gamePath_ & """ --assetsDir """ & gamePath & "assets"" --assetIndex 1.7.10 --accessToken myaccesstoken --userProperties {} --userType legacy --tweakClass cpw.mods.fml.common.launcher.FMLTweaker"
